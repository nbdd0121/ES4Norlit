...
inline uint64_t roundedSHR(uint64_t operand, uint8_t count) {
    operand >>= (count - 1);
    operand++;
    operand >>= 1;
    return operand;
...
uint8_t CountLeadingZeros(uint64_t operand) {
...
struct LongDouble {
    ...
    // val should be finite and positive
    LongDouble(double val) {
        union {
            uint64_t i;
            double d;
        } u;
    ...
    LongDouble(uint64_t frac, int exp) : frac(frac), exp(static_cast<int16_t>(exp)) {

    }

    LongDouble normalize() {
        uint8_t shift = CountLeadingZeros(frac);
        return{ frac << shift, exp - shift };
    }

    LongDouble operator -(const LongDouble& b) const {
        assert(exp == b.exp && frac > b.frac);
        return{ frac - b.frac, exp };
    }

    LongDouble operator *(const LongDouble& b) const {
        uint64_t aLo = frac & 0xFFFFFFFF, aHi = frac >> 32,
                 bLo = b.frac & 0xFFFFFFFF, bHi = b.frac >> 32;
        uint64_t aLoBLo = aLo * bLo, aLoBHi = aLo * bHi, aHiBLo = aHi * bLo, aHiBHi = aHi * bHi;
        uint64_t loPart = (aLoBLo >> 32) + (aLoBHi & 0xFFFFFFFF) + (aHiBLo & 0xFFFFFFFF) + 0x80000000;
        return{
            aHiBHi + (aLoBHi >> 32) + (aHiBLo >> 32) + (loPart >> 32),
            exp + b.exp + 64
        };
    }

    explicit operator double() {
        uint64_t frac = this->frac;
        int16_t exp = this->exp;
        uint8_t leadingZeros = CountLeadingZeros(frac);
        if (leadingZeros > 11) {
            frac <<= (leadingZeros - 11);
            exp -= (leadingZeros - 11);
        } else if (leadingZeros < 11) {
            frac = roundedSHR(frac, 11 - leadingZeros);
            exp += (11 - leadingZeros);
        }
        ...
        if (exp <= 0) {
            frac = roundedSHR(frac, 1 - exp);
            if ((frac & 0xFFF0000000000000) == 0) {
                u.i = frac;
            } else {
                throw "Out of bound";
            }
...
uint8_t norlit::util::CountDigits(uint64_t n) {
    if (n < 10) return 1;
    if (n < 100) return 2;
    if (n < 1000) return 3;
    if (n < 10000) return 4;
...
void norlit::util::DesembleDouble(double v, uint64_t& s, int16_t& n, uint8_t& k) {
    // This is Grisu2 with Alpha = 0, Gamma = 3

    // Convert v to long double
    LongDouble Dexact = LongDouble(v);
    // Half way between v and successor double value
    LongDouble upperBoundary = LongDouble((Dexact.frac << 1) + 1, Dexact.exp - 1).normalize();
    // Half way between v and predecessor double value
    LongDouble lowerBoundary = LongDouble(
                                   Dexact.frac == 0x8000000000000000ULL ?
                                   ((Dexact.frac << 2) - 1) << (Dexact.exp - 2 - upperBoundary.exp) :
                                   ((Dexact.frac << 1) - 1) << (Dexact.exp - 1 - upperBoundary.exp)
                                   , upperBoundary.exp);

    static const int q = 64, alpha = 0, gamma = 3;
    int mk = CalculateK(upperBoundary.exp + q, alpha, gamma);
    LongDouble c_mk = GetPower(mk);
    lowerBoundary = lowerBoundary * c_mk;
    upperBoundary = upperBoundary * c_mk;
    int power = -mk;
    ...
    if (upperBoundary.exp > 0) {
        // Left shift is not always safe. Save some bits
        // The exp will never be greater than 10, otherwise
        // it will be processed by CalculateK
        lowerBoundary.frac += 9;
        lowerBoundary.frac /= 10;
        upperBoundary.frac /= 10;
        power++;
    }

    uint64_t lowerBound = LongDoubleToInteger(lowerBoundary) + 1;
    uint64_t upperBound = LongDoubleToInteger(upperBoundary) - 1;

    // Reduce the length within boundary
    while (true) {
        // printf("%llu - %llu\n", lowerBound, upperBound);
        uint64_t newLB = (lowerBound + 9) / 10;
        uint64_t newUB = upperBound / 10;
        if (newLB > newUB) {
            break;
        }
        lowerBound = newLB;
    ...
    // Not sure whether plain average works
    lowerBound = Average(lowerBound, upperBound);

    // Remove trailing zeros
    while ((lowerBound % 10) == 0) {
        lowerBound /= 10;
        power++;
    }
...
double norlit::util::AssembleDouble(uint64_t s, int16_t n, uint8_t k) {
    n -= k;
    if (n > 343) {
        return std::numeric_limits<double>::infinity();
    } else if(n < -343 || s==0 ) {
    ...
    LongDouble number = LongDouble(s, 0).normalize();
    number = number * GetPower(n);
